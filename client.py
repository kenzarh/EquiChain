...
class Client: # Client's communication is synchronous: It can not send a request until its last request is answered.
    ...
    def broadcast_request(self,request_message,nodes_ids_list,sending_time,f): # This function is executed if the leader doesn't receive the request. It is then broadcasted to all the nodes
        ...
        while True:
            ...
            shard_id = received_message["shard"]
            f_shard = f[shard_id]
                            ...
                            if similar_replies == (f_shard+1):
                                ...
                                for i in range (settings.n):
                                    if i in (nodes_replies) and (nodes_replies[i]) == (accepted_response):
                                        ...
                                        import math
                                        if (settings.mean_delay_to_validate_accepted_block_per_node[i] == 1e9):
    ...
    def send_to_leader (self,request,leader_node_id,nodes_ids_list,f): # Sends a request to the leader and waits for f+1 similar answers
        leader_node_port = nodes_ports[leader_node_id]
        ...
        # The client sends the request to what it believes is the leader:
        sending_socket = socket.socket(socket.AF_INET, socket.SOCK_STREAM)
        host = socket.gethostname() 
        try:
            sending_socket.connect((host, leader_node_port))
            sending_socket.send(request_message)
        except:
             pass
        ...
        PBFT.delay_to_validate_transactions[request] = sending_time
        answered_nodes = [] # list of nodes ids that answered the request
        ...
        while True:
            ...
            shard_id = received_message["shard_id"]
            f_shard = f[shard_id]
            ...
            if (answering_node_id not in answered_nodes): # Ne rien faire si le noeud a déjà répondu   
                    ...
                    if (replies[str_response]>similar_replies):
                     ...
                     #print("Reply received!",request,similar_replies)
                    ...
                    if replies[str_response] >= (f_shard+1):
                            ...
                            #print("Reply received",self.client_id)
